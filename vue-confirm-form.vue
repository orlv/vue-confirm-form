    ...
    <div v-if="busy" class="vue-confirm-form" @click="cancel">
      <div class="vue-confirm-form-main" @click.stop>
        <div class="confirm-form-copy-paste-buttons">
          <div class="vue-confirm-clipboard margin-r1" @click="copyToClipboard">copy</div>
          <div class="vue-confirm-clipboard" @click="pasteFromClipboard">paste</div>
        </div>
            ...
            >
              ...
              <td v-else-if="typeof fieldValue === 'object'">
                ...
                <div v-for="(checkboxValue, value) in fieldValue" v-else :key="value">
                  <div
                    v-if="form[fieldName].includes(value)"
                    class="cursor-pointer"
                    @click="
                      form[fieldName].splice(
                        form[fieldName].findIndex((v) => v === value),
                        1
                      )
                    "
                  >
                  ...
                  <div v-else class="cursor-pointer" @click="form[fieldName].push(value)">
                    <span>☐</span>
                    <span>{{ value }}</span>
                  </div>
                </div>
...
export default {
  ...
  emits: ['on-select', 'on-change', 'cancel'],
  ...
  methods: {
    ...
    extract() {
      return Object.entries(this.fields).reduce((out, [field, inVal]) => {
        ...
        if (
          (typeof inVal === 'number' || (Array.isArray(inVal) && typeof inVal[0] === 'number')) &&
